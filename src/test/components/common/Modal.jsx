  ...
  return (
    ...
    max-h-[90vh] overflow-y-auto scrollbar-hide overscroll-contain`} // scrollbar-hide 추가!
      ...
      >
        ...
        <div className="flex justify-between items-center p-6 pb-[10px] sticky top-0 z-10 bg-inherit rounded-t-[30px]">
          <h2
            className={`text-2xl md:text-3xl font-black ${isDark ? "text-blue-500" : "text-blue-400"}`}
          >
            {title}
          </h2>
          <div className="flex items-center gap-4">
            ...
            >
              <div
                className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform ${isDark ? "translate-x-6" : "translate-x-0"} flex items-center justify-center text-[8px]`}
              >
                {isDark ? "🌙" : "☀️"}
              </div>
            </button>
            <button
              onClick={onClose}
              className="text-xl hover:rotate-90 transition-transform px-2"
            >
              ✕
            </button>
          </div>
        ...
        <div className="p-6 pt-9">
          {React.isValidElement(extra)
            ? React.cloneElement(extra, { isDark })
            : extra}
        </div>
      </div>
    </div>
